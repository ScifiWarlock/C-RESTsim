...
use_preset_obstacles = True  # Set to True to use preset obstacles, False for random ones
...
while dist(current_coordinates, end_coordinates) > final_radius:
   for p in range(len(path[:, 0])) :
       ...
       ax1.plot(path[:, 0][p], path[:, 1][p], 'b-', linewidth=2, label='Path')
       ax1.scatter(path[:, 0][p], path[:, 1][p], color='blue', s=30, label='Nodes')
